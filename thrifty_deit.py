...
class Layer_scale_init_Block_thrifty(nn.Module):
    ...
    def forward(self, x, norm1=None, norm2=None):
        x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x) if norm1 is None else norm1(x)))
        x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x) if norm2 is None else norm2(x)))
        return x
...
class vit_models_thrifty(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.norm1 = nn.ModuleList([norm_layer(embed_dim) for _ in range(depth)])
        self.norm2 = nn.ModuleList([norm_layer(embed_dim) for _ in range(depth)])

        self.depth = depth
        self.blocks = block_layers(
                    dim=embed_dim,
                    num_heads=num_heads,
                    mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias,
    ...
    def forward_features(self, x):
        ...
        for i in range(self.depth):
            x = self.blocks(x, norm1=self.norm1[i], norm2=self.norm2[i])
